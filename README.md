...
- IOS: open ```.xcworkspace``` with ``` Xcode ```. Right click ```Fonts``` folder. Add file to cc_manage_mobile. Select all fonts from ```src/app/assets/fonts```. DON'T TICK ```copy items if needed```. then save.
